...
function assert(comments) {
  ...
  expect(comments[0].tags[0].name).to.eql('spec.groovy');
...
describe('mkparse:', function() {
  ...
  it('should use groovy language', function(done) {
    var source = 'test/fixtures/lang/spec.groovy'
      , stream = parse.load(source, {rules: require('../../../lang/groovy')})
      , comments = [];
