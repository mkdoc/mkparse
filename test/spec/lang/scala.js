...
function assert(comments) {
  ...
  expect(comments[0].tags[0].name).to.eql('spec.scala');
...
describe('mkparse:', function() {
  ...
  it('should use scala language', function(done) {
    var source = 'test/fixtures/lang/spec.scala'
      , stream = parse.load(source, {rules: require('../../../lang/scala')})
      , comments = [];
