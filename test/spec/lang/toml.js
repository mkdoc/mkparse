...
function assert(comments) {
  ...
  expect(comments[0].tags[0].name).to.eql('spec.toml');
...
describe('mkparse:', function() {
  ...
  it('should use toml language', function(done) {
    var source = 'test/fixtures/lang/spec.toml'
      , stream = parse.load(source, {rules: require('../../../lang/toml')})
      , comments = [];
