var through = require('through3');

/**
 *  Parse comments from an array of lines.
 *
 *  When a comment is parsed an object is pushed to the stream 
 *  with an array of `lines`, the `rule` for the comment and the 
 *  `start` and `end` line numbers.
 *
 *  @module {constructor} Comment
 *  @param {Object} [opts] stream options.
 *  @option {Function} rules defines the comment rules.
 *  @option {Object} comments options to pass to the `rules` function.
 */
function Comment(opts) {
  opts = opts || {};

  var rules = opts.rules || require('../c');

  // call function to get map of match functions
  this.rules = rules(opts.comments);

  // line number
  this.line = 0;

  // result of attempt to match a line with a rule
  this.result = null;
}

/**
 *  Stream transform.
 *
 *  @private {function} transform
 *  @member Comment
 *  @param {Array} chunk lines to process.
 *  @param {String} encoding character encoding.
 *  @param {Function} callback function.
 */
function transform(chunk, encoding, cb) {
  var i
    , line;

  for(i = 0;i < chunk.length;i++) {
    line = chunk[i];

    this.line++;

    // test for comment start
    if(!this.result) {
      this.result = this.find(line); 
      if(this.result) {
        // handles comments that terminate on the same line
        if(this.result.finished) {
          this.reset();
          continue;
        }
      }
    // got an existing comment block
    }else{
      if(this.result.rule.end(line)) {
        if(this.result.rule.last) {
          this.result.lines.push(line);
        }
        this.result.end = this.result.rule.last ? this.line : (this.line - 1);
        this.push(this.result);
        this.reset();
      }else{
        this.result.lines.push(line);
      }
    }
  }
  cb();
}

/**
 *  @private
 */
function find(line) {
  var i
    , rule
    , match
    , ended
    , source = line
    , extra
    , result;

  for(i = 0;i < this.rules.length;i++) {
    rule = this.rules[i];
    if((match = rule.start.call(rule, line))) {
      result = {
        rule: rule,
        line: line,
        start: this.line,
        end: this.line,
        match: match,
        lines: [source]
      };

      // starting comment, does it finish on this line?
      if((ended = rule.end.call(rule, line))) {
        result.finished = ended;

        source = line.substr(
          match.index, (ended.index - match.index) + ended[0].length);
        extra = line.substr(ended.index + ended[0].length);

        result.lines = [source];
        this.push(result);

        // more comments to process on this line
        if(extra) {
          return find.call(this, extra);
        }
      // strip any leading data before start comment
      }else{
        source = line.substr(match.index);
      }

      result.lines = [source];
      return result;
    }
  }
}

/**
 *  @private
 */
function reset() {
  this.result = null;
}

Comment.prototype.find = find;
Comment.prototype.reset = reset;


module.exports = through.transform(transform, {ctor: Comment})
