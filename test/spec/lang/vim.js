...
function assert(comments) {
  ...
  expect(comments[0].tags[0].name).to.eql('spec.vim');
...
describe('mkparse:', function() {
  ...
  it('should use vim language', function(done) {
    var source = 'test/fixtures/lang/spec.vim'
      , stream = parse.load(source, {rules: require('../../../lang/vim')})
      , comments = [];
  ...
  it('should use vim language w/ options', function(done) {
    var source = 'test/fixtures/lang/spec.vim'
      , stream = parse.load(
          source, {
            rules: require('../../../lang/vim'),
