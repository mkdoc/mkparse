...
function assert(comments) {
  ...
  expect(comments[0].tags[0].name).to.eql('spec.coffee');
...
describe('mkparse:', function() {
  ...
  it('should use coffeescript language', function(done) {
    var source = 'test/fixtures/lang/spec.coffee'
      , stream = parse.load(
          source, {rules: require('../../../lang/coffeescript')})
      , comments = [];
