...
describe('mkparse:', function() {
  ...
  it('should parse inline comment block with default value', function(done) {
    var source = 'test/fixtures/value.js'
      , stream = parse.load(source)
      , expected = '// @private {String=bar} foo private constant';
    ...
    stream.once('comment', function(comment) {
      ...
      expect(comment.tags[0].value).to.eql('bar');
