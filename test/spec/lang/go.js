...
function assert(comments) {
  ...
  expect(comments[0].tags[0].name).to.eql('spec.go');
...
describe('mkparse:', function() {
  ...
  it('should use go language', function(done) {
    var source = 'test/fixtures/lang/spec.go'
      , stream = parse.load(source, {rules: require('../../../lang/go')})
      , comments = [];
